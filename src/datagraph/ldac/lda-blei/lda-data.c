...
lda_corpus_t* read_data(const char* data_filename)
{
    ...
    {
	c->docs = (lda_document_t*) realloc(c->docs, sizeof(lda_document_t)*(nd+1));
	...
	for (n = 0; n < length; n++)
	{
	    size_t nBytes __attribute((unused)) = fscanf(fileptr, "%10d:%10d", &word, &count);
	    word = word - OFFSET;
