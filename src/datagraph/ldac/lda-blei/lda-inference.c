...
double lda_inference(ldac_t *ldac, lda_document_t* doc, lda_model* model, double* var_gamma, double** phi)
{
    double converged = 1;
    ...
    while ((converged > ldac->VAR_CONVERGED) &&
           ((var_iter < ldac->VAR_MAX_ITER) || (ldac->VAR_MAX_ITER == 0)))
    {
	var_iter++;
	for (n = 0; n < doc->length; n++)
	{
...
double
compute_likelihood(lda_document_t* doc, lda_model* model, double** phi, double* var_gamma)
{
