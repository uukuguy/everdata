...
{
    uint32_t num_topics = model->num_topics;
    uint32_t num_terms = model->num_terms;
    uint32_t k, n;
...
void corpus_initialize_ss(lda_suffstats* ss, lda_model* model, lda_corpus_t* c)
{
    uint32_t num_topics = model->num_topics;
    uint32_t i, k, d, n;
    lda_document_t* doc;
...
lda_model* load_lda_model(const char* model_root)
{
    ...
    size_t nBytes __attribute__((unused)) = 0;
    nBytes = fscanf(fileptr, "num_topics %d\n", &num_topics);
    nBytes = fscanf(fileptr, "num_terms %d\n", &num_terms);
    nBytes = fscanf(fileptr, "alpha %f\n", &alpha);
    nBytes = fclose(fileptr);
