...
{
    if ( call_time < 1000 ){
        sprintf(sz_call_time, "%llu us", call_time);
    } else if ( call_time < 1000000 ) {
        sprintf(sz_call_time, "%llu ms", call_time / 1000);
    } else {
        uint64_t sec = call_time / 1000000;
        uint64_t ms = (call_time - sec * 1000000 ) / 1000;
        sprintf(sz_call_time, "%llu.%03llu sec", sec, ms);
    }
}
