...
static int log_enqueue(int prio, const char *fmt, va_list ap)
{
	...
	if (la->head <= la->tail &&
	    (len + sizeof(struct logmsg)) > (size_t)((char *)la->end - (char *)la->tail)) {
	...
	if (la->head > la->tail &&
	    (len + sizeof(struct logmsg)) > (size_t)((char *)la->head - (char *)la->tail)) {
...
static void log_syslog (void * buff)
{
	...
	if (la->fd >= 0){
		size_t nBytes __attribute__((unused)) = write(la->fd, (char *)&msg->str, strlen((char *)&msg->str));
    } else
...
static void dolog(int prio, const char *fmt, va_list ap)
{
	...
	if (la) {
		...
		size_t nBytes __attribute__((unused)) = write(STDERR_FILENO, p, len);
		fflush(stderr);
	}
}
